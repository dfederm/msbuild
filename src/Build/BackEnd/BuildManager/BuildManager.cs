    ...
    public class BuildManager : INodePacketHandler, IBuildComponentHost, IDisposable
    {
        ...
        private void HandleResult(int node, BuildResult result)
        {
            ...
            if (_buildSubmissions.TryGetValue(result.SubmissionId, out BuildSubmission buildSubmission))
            {
                ...
                ConfigurationMetadata submissionConfigurationMetadata = new(buildSubmission.BuildRequestData.ProjectFullPath, buildSubmission.BuildRequestData.GlobalPropertiesDictionary);
                BuildRequestConfiguration submissionConfiguration = _configCache.GetMatchingConfiguration(submissionConfigurationMetadata);
                if (submissionConfiguration.ConfigurationId == configuration.ConfigurationId && _projectCacheService.ShouldUseCache(submissionConfiguration))
                {
                    BuildEventContext buildEventContext = _projectStartedEvents.TryGetValue(result.SubmissionId, out BuildEventArgs buildEventArgs)
                        ...
                        : new BuildEventContext(result.SubmissionId, node, submissionConfiguration.Project?.EvaluationId ?? BuildEventContext.InvalidEvaluationId, submissionConfiguration.ConfigurationId, BuildEventContext.InvalidProjectContextId, BuildEventContext.InvalidTargetId, BuildEventContext.InvalidTaskId);
                    try
                    {
                        _projectCacheService.HandleBuildResultAsync(submissionConfiguration, result, buildEventContext, _executionCancellationTokenSource.Token).Wait();
                    }
